...
lazynet.core.getInstance = function()
	local instance = clr.Lazynet.AppMgr.LazynetAppManager.GetInstance();
	return instance;
end
...
lazynet.core.addService = function(service)
	local instance = lazynet.core.getInstance();
	instance.Context.Service.AddService(service)
end
...
-- 获取节点
lazynet.core.getNodeByName = function(name)
	local instance = lazynet.core.getInstance();
	local node = instance.Context.GetNodeByName(name);
	return node;
end

-- 添加节点
lazynet.core.addNode = function(name, ctx)
	local instance = lazynet.core.getInstance();
	instance.Context.AddNode(name, ctx);
end

-- 删除节点
lazynet.core.removeNode = function(ctx)
	local instance = lazynet.core.getInstance();
	instance.Context.RemoveNode(ctx);
end

-- 给节点发消息
lazynet.core.sendMessage = function(node, msg)
	local instance = lazynet.core.getInstance();
	instance.Context.SendMessage(node, msg);
end

-- 启动服务
lazynet.core.start = function(port, heartbeat, socketType)
	local instance = lazynet.core.getInstance();
	instance.Context.SetConfigInfo(port, heartbeat, socketType);
	instance.Start();
end
