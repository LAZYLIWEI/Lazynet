    ...
    public class LazynetAppServerHandler : ILazynetSocketEvent
    {
        ...
        private void CallService(LazynetHandlerContext ctx, LazynetMessage message)
        {
            try
            {
                ...
                {
                    this.Context.Log(LazynetLogLevel.Warn, this.Context.Name, message.RouteUrl + " action run slow ");
                }

            }
            catch (Exception ex)
            {
                this.Context.Log(LazynetLogLevel.Error, this.Context.Name, ex.ToString());
            }
        }
