...
lazynet.core.getInstance = function()
	local instance = clr.Lazynet.LogApp.LazynetAppManager.GetInstance();
	return instance;
end
...
lazynet.core.log = function(level, content)
	local instance = lazynet.core.getInstance();
	instance.Context.Log(level, lazynet.core.getName(), content)
end

lazynet.core.info = function(content)
	local instance = lazynet.core.getInstance();
	instance.Context.Log(lazynet.LogLevel.INFO, lazynet.core.getName(), content)
end

lazynet.core.print = function(name, content)
	local instance = lazynet.core.getInstance();
	instance.Context.Log(lazynet.LogLevel.INFO, name, content)
end
