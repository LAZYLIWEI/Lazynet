...
namespace Lazynet.GameAppMgr.Filter
{
    public class MyActionFilter : ILazynetServiceActionFilter
    {
        public void OnServiceExecuted(LazynetMessage message)
        {
            LazynetAppManager.GetInstance().Log(message.RouteUrl + " after", LazynetLogLevel.Info);
        }

        public void OnServiceExecuting(LazynetMessage message)
        {
            LazynetAppManager.GetInstance().Log(message.RouteUrl + " before", LazynetLogLevel.Info);
        }
    }
}
